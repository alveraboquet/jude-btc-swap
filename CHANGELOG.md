...
- Record judecoin wallet restore blockheight in state `SwapSetupCompleted` already.
  This solves issues where the CLI went offline after sending the BTC transaction, and the judecoin wallet restore blockheight being recorded after Alice locked the judecoin, resulting in the generated JUDE redeem wallet not detecting the transaction and reporting `No unlocked balance in the specified account`.
...
- `judecoin-recovery` command that can be used to print the judecoin address, private spend and view key so one can manually recover instances where the `judecoin-wallet-rpc` does not pick up the judecoin funds locked up by the ASB.
  Related issue: <https://github.com/comit-network/jude-btc-swap/issues/537>
...
- ASB and CLI can migrate their data to sqlite to store swaps and related data.
  This makes it easier to build applications on top of jude-btc-swap by enabling developers to read swap information directly from the database.
...
3. Run jude-btc-swap
   jude-btc swap will create a new sqlite database and use that from now on.

##### Preserve old data

It is possible to migrate critical data from the old db to the sqlite but there are many pitfalls.

1. Run jude-btc-swap as you would normally
   jude-btc-swap will try and automatically migrate your existing data to the new database.
...
- A `config` subcommand that prints the current configuration including the data directory location.
  This feature should alleviate difficulties users were having when finding where jude-btc-swap was storing data.
...
- A bug where the ASB erroneously transitioned into a punishable state upon a bitcoin transaction monitoring error.
  This could lead to a scenario where the ASB was neither able to punish, nor able to refund, so the JUDE could stay locked up forever while the CLI refunded the BTC.
...
- Add the ability to view the swap-cli bitcoin balance and withdraw.
  See issue <https://github.com/comit-network/jude-btc-swap/issues/694>.
...
- An issue where the connection between ASB and CLI would get closed prematurely.
  ...
  See issue <https://github.com/comit-network/jude-btc-swap/issues/648>.
- An issue where the ASB was unable to use the judecoin wallet in case `judecoin-wallet-rpc` has been restarted.
  In case no wallet is loaded when we try to interact with the `judecoin-wallet-rpc` daemon, we now load the correct wallet on-demand.
  See issue <https://github.com/comit-network/jude-btc-swap/issues/652>.
- An issue where swap protocol was getting stuck trying to submit the cancel transaction.
  ...
  See issues: <https://github.com/comit-network/jude-btc-swap/issues/709> <https://github.com/comit-network/jude-btc-swap/issues/688>, <https://github.com/comit-network/jude-btc-swap/issues/701>.
- An issue where the ASB withdraw one bitcoin UTXO at a time instead of the whole balance.
  See issue <https://github.com/comit-network/jude-btc-swap/issues/662>.
...
- A mandatory `--change-address` parameter to the CLI's `buy-jude` command.
  ...
  For more information see [#513](https://github.com/comit-network/jude-btc-swap/issues/513).
...
- An issue where the default judecoin node connection string would not work, because the public nodes were moved to a different domain.
  The default judecoind nodes were updated to use the [melo tool nodes](https://melo.tools/nodes.html).
...
- Refund command for the ASB that allows refunding a specific swap by id.
  Using the refund command to refund the JUDE locked by the ASB requires the CLI to first refund the BTC of the swap.
...
- Abort command for the ASB that allows safely aborting a specific swap.
  Only swaps in a state prior to locking JUDE can be safely aborted.
...
- An issue where both the ASB and the CLI point to the same default directory `jude-btc-swap` for storing data.
  The asb now uses `jude-btc-swap/asb` and the CLI `jude-btc-swap/cli` as default directory.
...
- An issue where the judecoin daemon port used by the `judecoin-wallet-rpc` could not be specified.
  The CLI parameter `--judecoin-daemon-host` was changed to `--judecoin-daemon-address` where host and port have to be specified.
...
- From this version on the CLI and ASB run on **mainnet** by default!
  When running either application with `--testnet` judecoin network defaults to `stagenet` and Bitcoin network to `testnet3`.
  This is a breaking change.
...
- An [issue](https://github.com/comit-network/jude-btc-swap/issues/353) where the `swap` CLI would fail on systems that were set to a locale different than English.
  A bad readiness check when waiting for `judecoin-wallet-rpc` to be ready caused the CLI to hang forever, preventing users from perform a swap.
...
[Unreleased]: https://github.com/comit-network/jude-btc-swap/compare/0.10.2...HEAD
[0.10.2]: https://github.com/comit-network/jude-btc-swap/compare/0.10.1...0.10.2
[0.10.1]: https://github.com/comit-network/jude-btc-swap/compare/0.10.0...0.10.1
[0.10.0]: https://github.com/comit-network/jude-btc-swap/compare/0.9.0...0.10.0
[0.9.0]: https://github.com/comit-network/jude-btc-swap/compare/0.8.3...0.9.0
[0.8.3]: https://github.com/comit-network/jude-btc-swap/compare/0.8.2...0.8.3
[0.8.2]: https://github.com/comit-network/jude-btc-swap/compare/0.8.1...0.8.2
[0.8.1]: https://github.com/comit-network/jude-btc-swap/compare/0.8.0...0.8.1
[0.8.0]: https://github.com/comit-network/jude-btc-swap/compare/0.7.0...0.8.0
[0.7.0]: https://github.com/comit-network/jude-btc-swap/compare/0.6.0...0.7.0
[0.6.0]: https://github.com/comit-network/jude-btc-swap/compare/0.5.0...0.6.0
[0.5.0]: https://github.com/comit-network/jude-btc-swap/compare/0.4.0...0.5.0
[0.4.0]: https://github.com/comit-network/jude-btc-swap/compare/v0.3...0.4.0
