...
use judecoin::cryptonote::hash::Hash;
use judecoin::util::ringct;
use judecoin::PublicKey;
use serde::de::DeserializeOwned;
use serde::{Deserialize, Serialize, Serializer};

#[jsonrpc_client::api(version = "2.0")]
pub trait judecoindRpc {
...
#[jsonrpc_client::implement(judecoindRpc)]
#[derive(Debug, Clone)]
pub struct Client {
...
impl Client {
    /// New local host judecoind RPC client.
        ...
        Res: DeserializeOwned,
    {
        let response = self
            .inner
            .post(url)
            .body(judecoin_epee_bin_serde::to_bytes(&request)?)
            .send()
            .await?;
        ...
        Ok(judecoin_epee_bin_serde::from_bytes(body)?)
    }
}
...
// We should be able to use judecoin-rs for this but it does not include all
// the fields.
#[derive(Clone, Debug, Deserialize)]
...
pub struct GetBlockResponse {
    #[serde(with = "judecoin_serde_hex_block")]
    pub blob: judecoin::Block,
}
...
mod judecoin_serde_hex_block {
    use super::*;
    use judecoin::consensus::Decodable;
    use serde::de::Error;
    use serde::{Deserialize, Deserializer};
    use std::io::Cursor;

    pub fn deserialize<'de, D>(deserializer: D) -> Result<judecoin::Block, D::Error>
    where
        D: Deserializer<'de>,
    {
        ...
        let block = judecoin::Block::consensus_decode(&mut cursor).map_err(D::Error::custom)?;
