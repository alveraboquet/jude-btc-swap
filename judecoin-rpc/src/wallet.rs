...
#[jsonrpc_client::implement(judecoinWalletRpc)]
#[derive(Debug, Clone)]
pub struct Client {
...
impl Client {
    /// Constructs a judecoin-wallet-rpc client with localhost endpoint.
    ...
    /// Constructs a judecoin-wallet-rpc client with `url` endpoint.
    ...
    /// Transfers `amount` judecoin from `account_index` to `address`.
...
pub struct Transfer {
    ...
    pub tx_key: Option<judecoin::PrivateKey>,
...
impl From<CheckTxKeyResponse> for CheckTxKey {
    fn from(response: CheckTxKeyResponse) -> Self {
        // Due to a bug in judecoind that causes check_tx_key confirmations
...
/// what the response of every RPC call is. Unfortunately, judecoind likes to
/// return empty objects instead of `null`s in certain cases. We use this struct
/// to all the "deserialization" to happily continue.
#[derive(Debug, Copy, Clone, Deserialize)]
pub struct Empty {}

fn opt_key_from_blank<'de, D>(deserializer: D) -> Result<Option<judecoin::PrivateKey>, D::Error>
where
    D: Deserializer<'de>,
{
