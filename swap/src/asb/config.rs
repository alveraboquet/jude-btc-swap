...
impl GetDefaults for Testnet {
    fn getConfigFileDefaults() -> Result<Defaults> {
        let defaults = Defaults {
            ...
            judecoin_wallet_rpc_url: Url::parse("http://127.0.0.1:38083/json_rpc")?,
...
impl GetDefaults for Mainnet {
    fn getConfigFileDefaults() -> Result<Defaults> {
        let defaults = Defaults {
            ...
            judecoin_wallet_rpc_url: Url::parse("http://127.0.0.1:18083/json_rpc")?,
            price_ticker_ws_url: Url::parse("wss://ws.kraken.com")?,
...
pub struct judecoin {
    ...
    #[serde(with = "crate::judecoin::network")]
    pub network: judecoin::Network,
}
...
pub fn query_user_for_initial_config(testnet: bool) -> Result<Config> {
    let (bitcoin_network, judecoin_network, defaults) = if testnet {
        tracing::info!("Running initial setup for testnet");

        let bitcoin_network = bitcoin::Network::Testnet;
        let judecoin_network = judecoin::Network::Stagenet;
        let defaults = Testnet::getConfigFileDefaults()?;

        (bitcoin_network, judecoin_network, defaults)
    } else {
        tracing::info!("Running initial setup for mainnet");
        let bitcoin_network = bitcoin::Network::Bitcoin;
        let judecoin_network = judecoin::Network::Mainnet;
    ...
    let judecoin_wallet_rpc_url = Input::with_theme(&ColorfulTheme::default())
        .with_prompt("Enter judecoin Wallet RPC URL or hit enter to use default")
        .default(defaults.judecoin_wallet_rpc_url)
        .interact_text()?;
    ...
    Ok(Config {
        ...
        judecoin: judecoin {
            wallet_rpc_url: judecoin_wallet_rpc_url,
            finality_confirmations: None,
            network: judecoin_network,
        },
        tor: TorConf {
...
mod tests {
    ...
    fn config_roundtrip_testnet() {
        ...
        let expected = Config {
            ...
            judecoin: judecoin {
                wallet_rpc_url: defaults.judecoin_wallet_rpc_url,
                finality_confirmations: None,
                network: judecoin::Network::Stagenet,
            },
            tor: Default::default(),
            maker: Maker {
    ...
    fn config_roundtrip_mainnet() {
        ...
        let expected = Config {
            ...
            judecoin: judecoin {
                wallet_rpc_url: defaults.judecoin_wallet_rpc_url,
                finality_confirmations: None,
                network: judecoin::Network::Mainnet,
            },
            tor: Default::default(),
            maker: Maker {
                min_buy_btc: bitcoin::Amount::from_btc(DEFAULT_MIN_BUY_AMOUNT).unwrap(),
