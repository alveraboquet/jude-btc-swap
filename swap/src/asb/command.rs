...
#[structopt(
    name = "asb",
    about = "Automated Swap Backend for swapping JUDE for BTC",
    author,
...
pub enum RawCommand {
    ...
    #[structopt(
        about = "Prints the Bitcoin and judecoin balance. Requires the judecoin-wallet-rpc to be running."
    )]
    Balance,
...
pub enum ManualRecovery {
    ...
    #[structopt(
        about = "Publishes the judecoin refund transaction. By default, a swap-state where the cancel transaction was already published will be enforced. This command requires the counterparty Bitcoin refund transaction and will error if it was not published yet. "
    )]
    Refund {
        #[structopt(flatten)]
    ...
    #[structopt(about = "Safely Abort requires the swap to be in a state prior to locking JUDE.")]
    SafelyAbort {
        #[structopt(
            long = "swap-id",
