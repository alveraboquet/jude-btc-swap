...
impl Database for SqliteDatabase {
    ...
    async fn insert_judecoin_address(&self, swap_id: Uuid, address: Address) -> Result<()> {
        let mut conn = self.pool.acquire().await?;
        ...
        sqlx::query!(
            r#"
        insert into judecoin_addresses (
            swap_id,
            address
            ) values (?, ?);
        "#,
    ...
    async fn get_judecoin_address(&self, swap_id: Uuid) -> Result<Address> {
        let mut conn = self.pool.acquire().await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_insert_load_judecoin_address() -> Result<()> {
        let db = setup_test_db().await?;

        let swap_id = Uuid::new_v4();
        let judecoin_address = "53gEuGZUhP9JMEBZoGaFNzhwEgiG7hwQdMCqFxiyiTeFPmkbt1mAoNybEUvYBKHcnrSgxnVWgZsTvRBaHBNXPa8tHiCU51a".parse()?;

        db.insert_judecoin_address(swap_id, judecoin_address).await?;

        let loaded_judecoin_address = db.get_judecoin_address(swap_id).await?;

        assert_eq!(judecoin_address, loaded_judecoin_address);
