...
use ::judecoin::{Address, Network, PrivateKey, PublicKey};
use anyhow::{Context, Result};
use judecoin_rpc::wallet::{BlockHeight, judecoinWalletRpc as _, Refreshed};
use judecoin_rpc::{jsonrpc, wallet};
use std::str::FromStr;
...
impl Wallet {
    ...
    pub async fn open_or_create(url: Url, name: String, env_config: Config) -> Result<Self> {
        ...
        if open_wallet_response.is_err() {
            client.create_wallet(name.clone(), "English".to_owned()).await.context(
                "Unable to create judecoin wallet, please ensure that the judecoin-wallet-rpc is available",
            )?;

            tracing::debug!(judecoin_wallet_name = %name, "Created judecoin wallet");
        } else {
            tracing::debug!(judecoin_wallet_name = %name, "Opened judecoin wallet");
        }
    ...
    pub async fn connect(client: wallet::Client, name: String, env_config: Config) -> Result<Self> {
        let main_address =
            judecoin::Address::from_str(client.get_address(0).await?.address.as_str())?;
        Ok(Self {
            inner: Mutex::new(client),
            network: env_config.judecoin_network,
            name,
            main_address,
            sync_interval: env_config.judecoin_sync_interval(),
        })
    }
            ...
            Ok(_) => match wallet.sweep_all(self.main_address.to_string()).await {
                Ok(sweep_all) => {
                    for tx in sweep_all.tx_hash_list {
                        tracing::info!(
                            %tx,
                            judecoin_address = %self.main_address,
                            "judecoin transferred back to default wallet");
                    }
                }
                Err(error) => {
                    tracing::warn!(
                        address = %self.main_address,
                        "Failed to transfer judecoin to default wallet: {:#}", error
                    );
                }
            },
            Err(error) => {
    ...
    pub async fn transfer(&self, request: TransferRequest) -> Result<TransferProof> {
        ...
        tracing::debug!(
            ...
            to = %public_spend_key,
            tx_id = %res.tx_hash,
            "Successfully initiated judecoin transfer"
        );
    ...
    pub async fn watch_for_transfer(&self, request: WatchRequest) -> Result<(), InsufficientFunds> {
        ...
        tracing::info!(
            ...
            "Waiting for judecoin transaction finality"
        );
...
async fn wait_for_confirmations<C: judecoin_rpc::wallet::judecoinWalletRpc<reqwest::Client> + Sync>(
    client: &Mutex<C>,
    ...
    while seen_confirmations < conf_target {
            ...
            .await
        {
            ...
            Err(jsonrpc::Error::JsonRpc(jsonrpc::JsonRpcError { code: -1, .. })) => {
                tracing::warn!(%txid, "`judecoin-wallet-rpc` failed to fetch transaction, may need to be restarted");
                continue;
            }
        ...
        if tx.confirmations > seen_confirmations {
            ...
            tracing::info!(
                ...
                "Received new confirmation for judecoin lock tx"
            );
        }
    }
...
mod tests {
    ...
    use judecoin_rpc::wallet::CheckTxKey;
    ...
    async fn given_exact_confirmations_does_not_fetch_tx_again() {
        ...
        let result = wait_for_confirmations(
            ...
            TransferProof::new(TxHash("<FOO>".to_owned()), PrivateKey {
                scalar: crate::judecoin::Scalar::random(&mut rand::thread_rng())
            }),
    ...
    async fn visual_log_check() {
        ...
        assert_eq!(
            writer.captured(),
            r" INFO swap::judecoin::wallet: Received new confirmation for judecoin lock tx txid=<FOO> seen_confirmations=1 needed_confirmations=5
 INFO swap::judecoin::wallet: Received new confirmation for judecoin lock tx txid=<FOO> seen_confirmations=3 needed_confirmations=5
 INFO swap::judecoin::wallet: Received new confirmation for judecoin lock tx txid=<FOO> seen_confirmations=5 needed_confirmations=5
"
        );
    }
    ...
    async fn reopens_wallet_in_case_not_available() {
        ...
        assert_eq!(
            writer.captured(),
            r" INFO swap::judecoin::wallet: Received new confirmation for judecoin lock tx txid=<FOO> seen_confirmations=1 needed_confirmations=5
DEBUG swap::judecoin::wallet: Opening wallet `foo-wallet` because no wallet is loaded
 INFO swap::judecoin::wallet: Received new confirmation for judecoin lock tx txid=<FOO> seen_confirmations=3 needed_confirmations=5
 INFO swap::judecoin::wallet: Received new confirmation for judecoin lock tx txid=<FOO> seen_confirmations=5 needed_confirmations=5
"
        );
    ...
    impl judecoin_rpc::wallet::judecoinWalletRpc<reqwest::Client> for DummyClient {
        async fn open_wallet(
            &self,
            _: String,
        ) -> Result<wallet::WalletOpened, judecoin_rpc::jsonrpc::Error<reqwest::Error>> {
            self.open_wallet_invocations.fetch_add(1, Ordering::SeqCst);

            Ok(judecoin_rpc::wallet::Empty {})
        }
        ...
        async fn check_tx_key(
            ...
            _: String,
            _: String,
        ) -> Result<wallet::CheckTxKey, judecoin_rpc::jsonrpc::Error<reqwest::Error>> {
            let index = self.check_tx_key_invocations.fetch_add(1, Ordering::SeqCst);

            self.check_tx_key_responses[index as usize]
                .clone()
                .map_err(|(code, message)| {
                    judecoin_rpc::jsonrpc::Error::JsonRpc(judecoin_rpc::jsonrpc::JsonRpcError {
                        code,
                        message,
                        data: None,
                    })
        ...
        async fn send_request<P>(
            &self,
            _: String,
        ) -> Result<judecoin_rpc::jsonrpc::Response<P>, reqwest::Error>
        where
