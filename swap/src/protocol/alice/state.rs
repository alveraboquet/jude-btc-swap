...
use crate::judecoin::wallet::{TransferRequest, WatchRequest};
use crate::judecoin::TransferProof;
use crate::judecoin_ext::ScalarExt;
...
impl fmt::Display for AliceState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AliceState::XmrLockTransactionSent { .. } => write!(f, "jude lock transaction sent"),
            AliceState::XmrLocked { .. } => write!(f, "jude is locked"),
            AliceState::XmrLockTransferProofSent { .. } => {
                write!(f, "jude lock transfer proof sent")
            }
            ...
            AliceState::XmrRefunded => write!(f, "jude is refunded"),
...
pub struct State0 {
    a: bitcoin::SecretKey,
    s_a: judecoin::Scalar,
    v_a: judecoin::PrivateViewKey,
    S_a_judecoin: judecoin::PublicKey,
    S_a_bitcoin: bitcoin::PublicKey,
    dleq_proof_s_a: CrossCurveDLEQProof,
    btc: bitcoin::Amount,
    jude: judecoin::Amount,
...
impl State0 {
        ...
        R: RngCore + CryptoRng,
    {
        ...
        let v_a = judecoin::PrivateViewKey::new_random(rng);

        let s_a = judecoin::Scalar::random(rng);
        let (dleq_proof_s_a, (S_a_bitcoin, S_a_judecoin)) = CROSS_CURVE_PROOF_SYSTEM.prove(&s_a, rng);
    ...
    pub fn receive(self, msg: Message0) -> Result<(Uuid, State1)> {
        let valid = CROSS_CURVE_PROOF_SYSTEM.verify(
            ...
            (
                ...
                msg.S_b_judecoin
                    .point
                    .decompress()
                    .ok_or_else(|| anyhow!("S_b is not a judecoin curve point"))?,
            ),
        );
        ...
        Ok((msg.swap_id, State1 {
            ...
            B: msg.B,
            s_a: self.s_a,
            S_a_judecoin: self.S_a_judecoin,
            S_a_bitcoin: self.S_a_bitcoin,
            S_b_judecoin: msg.S_b_judecoin,
...
pub struct State1 {
    ...
    S_a_judecoin: judecoin::PublicKey,
    S_a_bitcoin: bitcoin::PublicKey,
    S_b_judecoin: judecoin::PublicKey,
    S_b_bitcoin: bitcoin::PublicKey,
    v: judecoin::PrivateViewKey,
    v_a: judecoin::PrivateViewKey,
    dleq_proof_s_a: CrossCurveDLEQProof,
    btc: bitcoin::Amount,
    jude: judecoin::Amount,
...
impl State1 {
    ...
    pub fn receive(self, msg: Message2) -> Result<State2> {
        ...
        Ok(State2 {
            ...
            B: self.B,
            s_a: self.s_a,
            S_b_judecoin: self.S_b_judecoin,
...
impl State2 {
    pub fn next_message(&self) -> Message3 {
        ...
        // Alice encsigns the refund transaction(bitcoin) digest with Bob's judecoin
...
impl State3 {
    ...
    pub fn lock_jude_transfer_request(&self) -> TransferRequest {
        let S_a = judecoin::PublicKey::from_private_key(&judecoin::PrivateKey { scalar: self.s_a });

        let public_spend_key = S_a + self.S_b_judecoin;
    ...
    pub fn lock_jude_watch_request(
        &self,
        transfer_proof: TransferProof,
        conf_target: u64,
    ) -> WatchRequest {
    ...
    pub fn extract_judecoin_private_key(
        &self,
        published_refund_tx: bitcoin::Transaction,
    ) -> Result<judecoin::PrivateKey> {
        self.tx_refund().extract_judecoin_private_key(
            published_refund_tx,
            self.s_a,
            self.a.clone(),
            self.S_b_bitcoin,
    ...
    pub async fn refund_jude(
        &self,
        judecoin_wallet: &judecoin::Wallet,
        judecoin_wallet_restore_blockheight: BlockHeight,
        file_name: String,
        spend_key: judecoin::PrivateKey,
        transfer_proof: TransferProof,
        ...
        // Ensure that the JUDE to be refunded are spendable by awaiting 10 confirmations
        // on the lock transaction
        judecoin_wallet
            .watch_for_transfer(self.lock_jude_watch_request(transfer_proof, 10))
            .await?;
