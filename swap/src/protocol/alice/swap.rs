    ...
    LR: LatestRate,
{
    ...
    Ok(match state {
        ...
        AliceState::BtcLocked { state3 } => {
            match state3.expired_timelocks(bitcoin_wallet).await? {
                ExpiredTimelocks::None => {
                    ...
                    let transfer_proof = judecoin_wallet
                        .transfer(state3.lock_jude_transfer_request())
                        .await?;
        ...
        AliceState::XmrLockTransactionSent {
            ...
            ExpiredTimelocks::None => {
                judecoin_wallet
                    .watch_for_transfer(state3.lock_jude_watch_request(transfer_proof.clone(), 1))
                    .await
                    .with_context(|| {
                        format!(
                            "Failed to watch for transfer of JUDE in transaction {}",
        ...
        AliceState::BtcCancelled {
            ...
            select! {
                seen_refund = tx_refund_status.wait_until_seen() => {
                    ...
                    let spend_key = state3.extract_judecoin_private_key(published_refund_tx)?;
