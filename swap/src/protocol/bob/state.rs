...
use crate::judecoin;
use crate::judecoin::wallet::WatchRequest;
...
impl fmt::Display for BobState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            BobState::XmrLocked(..) => write!(f, "jude is locked"),
            ...
            BobState::XmrRedeemed { .. } => write!(f, "jude is redeemed"),
...
pub struct State0 {
    ...
    min_judecoin_confirmations: u64,
...
impl State0 {
    ...
    pub fn new<R: RngCore + CryptoRng>(
        ...
        let s_b = judecoin::Scalar::random(rng);
        let v_b = judecoin::PrivateViewKey::new_random(rng);

        let (dleq_proof_s_b, (S_b_bitcoin, S_b_judecoin)) = CROSS_CURVE_PROOF_SYSTEM.prove(&s_b, rng);
        ...
        D: BatchDatabase,
    {
        let valid = CROSS_CURVE_PROOF_SYSTEM.verify(
            ...
            (
                ...
                msg.S_a_judecoin
                    .point
                    .decompress()
                    .ok_or_else(|| anyhow!("S_a is not a judecoin curve point"))?,
            ),
        );
        ...
        Ok(State1 {
            ...
            tx_lock,
            min_judecoin_confirmations: self.min_judecoin_confirmations,
...
impl State3 {
    pub fn lock_jude_watch_request(&self, transfer_proof: TransferProof) -> WatchRequest {
        let S_b_judecoin =
            judecoin::PublicKey::from_private_key(&judecoin::PrivateKey::from_scalar(self.s_b));
        let S = self.S_a_judecoin + S_b_judecoin;

        WatchRequest {
            public_spend_key: S,
            public_view_key: self.v.public(),
            transfer_proof,
            conf_target: self.min_judecoin_confirmations,
            expected: self.jude,
        }
    }

    pub fn jude_locked(self, judecoin_wallet_restore_blockheight: BlockHeight) -> State4 {
        State4 {
            A: self.A,
            b: self.b,
            s_b: self.s_b,
...
impl State4 {
    ...
    pub async fn watch_for_redeem_btc(&self, bitcoin_wallet: &bitcoin::Wallet) -> Result<State5> {
        ...
        let s_a = judecoin::private_key_from_secp256k1_scalar(s_a.into());
        ...
        Ok(State5 {
            ...
            v: self.v,
            tx_lock: self.tx_lock.clone(),
            judecoin_wallet_restore_blockheight: self.judecoin_wallet_restore_blockheight,
        })
    }
...
pub struct State5 {
    ...
    s_a: judecoin::PrivateKey,
    s_b: judecoin::Scalar,
    v: judecoin::PrivateViewKey,
    tx_lock: bitcoin::TxLock,
    pub judecoin_wallet_restore_blockheight: BlockHeight,
}

impl State5 {
    pub fn jude_keys(&self) -> (judecoin::PrivateKey, judecoin::PrivateViewKey) {
        let s_b = judecoin::PrivateKey { scalar: self.s_b };
        let s = self.s_a + s_b;
