...
async fn main() -> Result<()> {
    ...
    match cmd {
        Command::BuyXmr {
            ...
            .await?;
            let (judecoin_wallet, _process) =
                init_judecoin_wallet(data_dir, judecoin_daemon_address, env_config).await?;
            ...
            db.insert_judecoin_address(swap_id, judecoin_receive_address)
                .await?;
        ...
        Command::Config => {
            ...
            tracing::info!(path=%format!("{}/judecoin", data_dir.display()), "judecoin-wallet-rpc directory");
        ...
        Command::Resume {
            ...
            let judecoin_receive_address = db.get_judecoin_address(swap_id).await?;
        ...
        Command::judecoinRecovery { swap_id } => {
            let db = open_db(data_dir.join("sqlite")).await?;
            ...
            match swap_state {
                ...
                | BobState::XmrRedeemed { .. } => {
                    bail!("Cannot print judecoin recovery information in state {}, only possible for BtcRedeemed", swap_state)
                }
                BobState::BtcRedeemed(state5) => {
                    let (spend_key, view_key) = state5.jude_keys();

                    let address = judecoin::Address::standard(
                        env_config.judecoin_network,
                        judecoin::PublicKey::from_private_key(&spend_key),
                        judecoin::PublicKey::from(view_key.public()),
                    );
...
async fn init_judecoin_wallet(
    data_dir: PathBuf,
    judecoin_daemon_address: String,
    env_config: Config,
) -> Result<(judecoin::Wallet, judecoin::WalletRpcProcess)> {
    let network = env_config.judecoin_network;

    const JUDECOIN_BLOCKCHAIN_MONITORING_WALLET_NAME: &str = "swap-tool-blockchain-monitoring-wallet";

    let judecoin_wallet_rpc = judecoin::WalletRpc::new(data_dir.join("judecoin")).await?;

    let judecoin_wallet_rpc_process = judecoin_wallet_rpc
        .run(network, judecoin_daemon_address.as_str())
        .await?;

    let judecoin_wallet = judecoin::Wallet::open_or_create(
        judecoin_wallet_rpc_process.endpoint(),
        JUDECOIN_BLOCKCHAIN_MONITORING_WALLET_NAME.to_string(),
        env_config,
    )
    .await?;

    Ok((judecoin_wallet, judecoin_wallet_rpc_process))
}
