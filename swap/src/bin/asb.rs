...
use swap::judecoin::Amount;
use swap::network::rendezvous::XmrBtcNamespace;
use swap::network::swarm;
use swap::protocol::alice::{run, AliceState};
use swap::seed::Seed;
use swap::tor::AuthenticatedClient;
use swap::{asb, bitcoin, kraken, judecoin, tor};
...
async fn main() -> Result<()> {
    ...
    if config.judecoin.network != env_config.judecoin_network {
        bail!(format!(
            "Expected judecoin network in config file to be {:?} but was {:?}",
            env_config.judecoin_network, config.judecoin.network
        ));
    }
    ...
    match cmd {
        Command::Start { resume_only } => {
            ...
            let judecoin_wallet = init_judecoin_wallet(&config, env_config).await?;

            let bitcoin_balance = bitcoin_wallet.balance().await?;
            tracing::info!(%bitcoin_balance, "Initialized Bitcoin wallet");

            let judecoin_balance = judecoin_wallet.get_balance().await?;
            if judecoin_balance == Amount::ZERO {
                let judecoin_address = judecoin_wallet.get_main_address();
                tracing::warn!(
                    %judecoin_address,
                    "The judecoin balance is 0, make sure to deposit funds at",
                )
            } else {
                tracing::info!(%judecoin_balance, "Initialized judecoin wallet");
            }
        ...
        Command::Refund { swap_id } => {
            ...
            tracing::info!("judecoin successfully refunded");
        }
...
async fn init_judecoin_wallet(
    config: &Config,
    env_config: swap::env::Config,
) -> Result<judecoin::Wallet> {
    tracing::debug!("Opening judecoin wallet");
    let wallet = judecoin::Wallet::open_or_create(
        config.judecoin.wallet_rpc_url.clone(),
