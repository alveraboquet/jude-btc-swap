...
use libp2p::PeerId;
use judecoin_harness::{image, judecoin};
...
use swap::seed::Seed;
use swap::{asb, bitcoin, cli, env, judecoin};
    ...
    C: GetConfig,
{
    ...
    let _guard = tracing_subscriber::fmt()
        .with_env_filter("warn,swap=debug,judecoin_harness=debug,judecoin_rpc=debug,bitcoin_harness=info,testcontainers=info") // add `reqwest::connect::verbose=trace` if you want to logs of the RPC clients
        .with_test_writer()
        .set_default();

    let env_config = C::get_config();

    let (judecoin, containers) = init_containers(&cli).await;
    judecoin.init_miner().await.unwrap();

    let btc_amount = bitcoin::Amount::from_sat(1_000_000);
    let jude_amount = judecoin::Amount::from_judecoin(btc_amount.as_btc() / FixedRate::RATE).unwrap();

    let alice_starting_balances =
        StartingBalances::new(bitcoin::Amount::ZERO, jude_amount, Some(10));
    ...
    let (alice_bitcoin_wallet, alice_judecoin_wallet) = init_test_wallets(
        JUDECOIN_WALLET_NAME_ALICE,
    ...
    let (alice_handle, alice_swap_handle) = start_alice(
        ...
        alice_judecoin_wallet.clone(),
    )
    .await;

    let bob_seed = Seed::random().unwrap();
    let bob_starting_balances = StartingBalances::new(btc_amount * 10, judecoin::Amount::ZERO, None);

    let (bob_bitcoin_wallet, bob_judecoin_wallet) = init_test_wallets(
        JUDECOIN_WALLET_NAME_BOB,
    ...
    let bob_params = BobParams {
        ...
        judecoin_wallet: bob_judecoin_wallet.clone(),
        alice_address: alice_listen_address.clone(),
        alice_peer_id: alice_handle.peer_id,
        env_config,
    };

    judecoin.start_miner().await.unwrap();
...
async fn init_containers(cli: &Cli) -> (judecoin, Containers<'_>) {
    ...
    let (judecoin, judecoind_container, judecoin_wallet_rpc_containers) =
        judecoin::new(&cli, vec![JUDECOIN_WALLET_NAME_ALICE, JUDECOIN_WALLET_NAME_BOB])
            .await
            .unwrap();
    ...
    (judecoin, Containers {
        ...
        bitcoind,
        judecoind_container,
        judecoin_wallet_rpc_containers,
        electrs,
    })
}
...
async fn init_test_wallets(
    ...
    seed: &Seed,
    env_config: Config,
) -> (Arc<bitcoin::Wallet>, Arc<judecoin::Wallet>) {
    judecoin
        .init_wallet(
            name,
            starting_balances
    ...
    let jude_wallet = swap::judecoin::Wallet::connect(
        judecoin.wallet(name).unwrap().client().clone(),
        name.to_string(),
        env_config,
    )
    .await
...
const JUDECOIN_WALLET_NAME_BOB: &str = "bob";
const JUDECOIN_WALLET_NAME_ALICE: &str = "alice";
const BITCOIN_TEST_WALLET_NAME: &str = "testwallet";

#[derive(Debug, Clone)]
pub struct StartingBalances {
    pub jude: judecoin::Amount,
    pub jude_outputs: Vec<judecoin::Amount>,
    pub btc: bitcoin::Amount,
}

impl StartingBalances {
    /// If judecoin_outputs is specified the judecoin balance will be:
    /// judecoin_outputs * new_jude = self_jude
    pub fn new(btc: bitcoin::Amount, jude: judecoin::Amount, judecoin_outputs: Option<u64>) -> Self {
        match judecoin_outputs {
            None => {
                if jude == judecoin::Amount::ZERO {
                    return Self {
                        jude,
                        jude_outputs: vec![],
                        btc,
                ...
                Self {
                    jude,
                    jude_outputs: vec![jude],
                    btc,
                }
            }
            Some(outputs) => {
                let mut jude_outputs = Vec::new();
                let mut sum_jude = judecoin::Amount::ZERO;

                for _ in 0..outputs {
                    jude_outputs.push(jude);
                    sum_jude = sum_jude + jude;
                }
...
impl BobParams {
    pub async fn new_swap_from_db(&self, swap_id: Uuid) -> Result<(bob::Swap, cli::EventLoop)> {
        ...
        let swap = bob::Swap::from_db(
            ...
            self.judecoin_wallet.clone(),
            self.env_config,
            handle,
            self.judecoin_wallet.get_main_address(),
        )
        .await?;
...
pub struct TestContext {
    ...
    alice_judecoin_wallet: Arc<judecoin::Wallet>,
    ...
    bob_judecoin_wallet: Arc<judecoin::Wallet>,
}
...
impl TestContext {
    pub async fn restart_alice(&mut self) {
        ...
        let (alice_handle, alice_swap_handle) = start_alice(
            ...
            self.alice_judecoin_wallet.clone(),
        )
        .await;
    ...
    pub async fn assert_alice_redeemed(&mut self, state: AliceState) {
        ...
        assert_eventual_balance(
            self.alice_judecoin_wallet.as_ref(),
            Ordering::Less,
            self.alice_redeemed_jude_balance(),
        )
        .await
        .unwrap();
    }
    ...
    pub async fn assert_alice_refunded(&mut self, state: AliceState) {
        ...
        assert_eventual_balance(
            self.alice_judecoin_wallet.as_ref(),
            Ordering::Greater,
            self.alice_refunded_jude_balance(),
        )
        .await
        .unwrap();
    }
    ...
    pub async fn assert_alice_punished(&self, state: AliceState) {
        ...
        assert_eventual_balance(
            self.alice_judecoin_wallet.as_ref(),
            Ordering::Less,
            self.alice_punished_jude_balance(),
        )
        .await
        .unwrap();
    }
    ...
    pub async fn assert_bob_redeemed(&self, state: BobState) {
        ...
        self.bob_judecoin_wallet.re_open().await.unwrap();

        assert_eventual_balance(
            self.bob_judecoin_wallet.as_ref(),
            Ordering::Greater,
            self.bob_redeemed_jude_balance(),
        )
        .await
        .unwrap();
    }
    ...
    pub async fn assert_bob_refunded(&self, state: BobState) {
        ...
        assert_eventual_balance(
            self.bob_judecoin_wallet.as_ref(),
            Ordering::Equal,
            self.bob_refunded_jude_balance(),
        )
        .await
        .unwrap();
    }
    ...
    pub async fn assert_bob_punished(&self, state: BobState) {
        ...
        assert_eventual_balance(
            self.bob_judecoin_wallet.as_ref(),
            Ordering::Equal,
            self.bob_punished_jude_balance(),
        )
        .await
        .unwrap();
    }

    fn alice_redeemed_jude_balance(&self) -> judecoin::Amount {
        self.alice_starting_balances.jude - self.jude_amount
    }
    ...
    fn bob_redeemed_jude_balance(&self) -> judecoin::Amount {
        self.bob_starting_balances.jude
    }
    ...
    async fn bob_redeemed_btc_balance(&self, state: BobState) -> Result<bitcoin::Amount> {
        ...
        let lock_tx_id = if let BobState::XmrRedeemed { tx_lock_id } = state {
            ...
            bail!("Bob in not in jude redeemed state: {:?}", state);
        };
    ...
    fn alice_refunded_jude_balance(&self) -> judecoin::Amount {
        self.alice_starting_balances.jude - self.jude_amount
    }
    ...
    fn bob_refunded_jude_balance(&self) -> judecoin::Amount {
        self.bob_starting_balances.jude
    }

    fn alice_punished_jude_balance(&self) -> judecoin::Amount {
        self.alice_starting_balances.jude - self.jude_amount
    }
    ...
    fn bob_punished_jude_balance(&self) -> judecoin::Amount {
        self.bob_starting_balances.jude
    }
...
#[async_trait]
impl Wallet for judecoin::Wallet {
    type Amount = judecoin::Amount;
...
struct Containers<'a> {
    ...
    judecoind_container: Container<'a, Cli, image::judecoind>,
    judecoin_wallet_rpc_containers: Vec<Container<'a, Cli, image::judecoinWalletRpc>>,
...
pub mod alice_run_until {
    ...
    pub fn is_jude_lock_transaction_sent(state: &AliceState) -> bool {
