...
This quickstart guide assumes that you are running the software on testnet (i.e. Bitcoin testnet3 and judecoin stagenet):

1. Download [latest release](https://github.com/comit-network/jude-btc-swap/releases/latest) of the `asb` binary
2. Ensure that you have the judecoin Wallet RPC running with `--wallet-dir` and `--disable-rpc-login`:
   1. `judecoin-wallet-rpc --stagenet --daemon-host STAGENET-NODE-URL --rpc-bind-port STAGENET-NODE-PORT --disable-rpc-login --wallet-dir PATH/TO/WALLET/DIR`
3. Run the ASB in terminal: `./asb --testnet start`
4. Follow the setup wizard in the terminal

Public judecoin nodes for running the judecoin Wallet RPC can be found [here](https://melo.tools/nodes.html).

Run `./asb --help` for more information.

### Running on mainnet

For running the ASB on mainnet you will have to change you `judecoin-wallet-rpc` setup to mainnet.

It is recommended that you run your own judecoin and Bitcoin node when running on mainnet.
It is possible to plug into public blockchain nodes but be aware that you might lose some privacy doing so.
Public judecoin mainnet nodes can be found [here](https://judecoinworld.com/#nodes).
...
The CLI user is buying JUDE (i.e. receives JUDE, sends BTC), the ASB service provider is selling JUDE (i.e. sends JUDE, receives BTC).
...
In order to understand the different components of the ASB and CLI better here is a component diagram showcasing the ASB and CLI setup using public Bitcoin and judecoin infrastructure:

![Service Provider scenarios](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/comit-network/jude-btc-swap/363ce1cdf6fe6478736ff91e1458d650c2319248/docs/asb/diagrams/cli-asb-components-asb-pub-nodes.puml)

Contrary, here is a diagram that showcases a service provider running it's own blockchain infrastructure for the ASB:

![Service Provider scenarios](http://www.plantuml.com/plantuml/proxy?cache=no&src=https://raw.githubusercontent.com/comit-network/jude-btc-swap/363ce1cdf6fe6478736ff91e1458d650c2319248/docs/asb/diagrams/cli-asb-components-asb-self-hosted.puml)
...
#### judecoin Wallet Setup

The ASB uses the running judecoin wallet RPC to create / open judecoin wallets.
Currently you cannot connect to an existing judecoin wallet, but the ASB will create the wallet `asb-wallet` upon intial startup.
In order to accept trades with a CLI you will have to send JUDE to that wallet.
...
The `JUDE<>BTC` price is currently determined by the price from the central exchange Kraken.
...
A CLI can connect to the ASB at any time and request a quote for buying JUDE.
...
The ASB can then refund the judecoin which will be automatically transferred back to the `asb-wallet`.

The punish scenario is a scenario where the CLI does not refund and hence the ASB cannot refund the judecoin.
After a second timelock expires the ASB will automatically punish the CLI user by taking the Bitcoin.

More information about the protocol in this [presentation](https://youtu.be/Jj8rd4WOEy0) and this [blog post](https://comit.network/blog/2020/10/06/judecoin-bitcoin).
...
If the ASB has insufficient judecoin funds to accept a swap the swap setup is rejected.
Note that there is currently no notification service implemented for low funds.
The ASB provider has to monitor judecoin funds to make sure the ASB still has liquidity.
