use judecoin_harness::{judecoin, judecoinWalletRpc};
use judecoin_rpc::wallet::judecoinWalletRpc as _;
...
async fn fund_transfer_and_check_tx_key() {
    ...
    let tc = Cli::default();
    let (judecoin, _judecoind_container, _wallet_containers) =
        judecoin::new(&tc, vec!["alice", "bob"]).await.unwrap();
    let alice_wallet = judecoin.wallet("alice").unwrap();
    let bob_wallet = judecoin.wallet("bob").unwrap();

    judecoin.init_miner().await.unwrap();
    judecoin.init_wallet("alice", vec![fund_alice]).await.unwrap();
    judecoin.init_wallet("bob", vec![fund_bob]).await.unwrap();
...
async fn wait_for_wallet_to_catch_up(wallet: &judecoinWalletRpc, expected_balance: u64) {
    let max_retry = 15;
    let mut retry = 0;
    loop {
        retry += 1;
